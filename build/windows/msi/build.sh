...
if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
  PRODUCT_NAME="BitForge - Insiders"
  PRODUCT_CODE="BitForgeInsiders"
  ...
  PRODUCT_NAME="BitForge"
  PRODUCT_CODE="BitForge"
...
if [[ -z "${1}" ]]; then
	OUTPUT_BASE_FILENAME="BitForge-${VSCODE_ARCH}-${RELEASE_VERSION}"
else
	OUTPUT_BASE_FILENAME="BitForge-${VSCODE_ARCH}-${1}-${RELEASE_VERSION}"
fi
